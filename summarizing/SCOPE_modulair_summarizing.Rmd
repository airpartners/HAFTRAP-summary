...
This file is to summarize Modulair Data for SCOPE deployments
...
modulair_df_long <- modulair_df %>%
  ...
  pivot_longer(!c(date, environment, dep_id, case), 
               names_to = "p_type", values_to = "reading") %>%
  # Drop NaN, inf values for summary calculation
  filter(is.finite(reading)) 

# Calculate summary statistics for each deployment
deployment_summary <- modulair_df_long %>%
  my_summarise(c("dep_id", "case", "environment", "p_type"))
  
# Calculate summary statistics over all the data
overall_summary <- modulair_df_long %>%
  my_summarise(c("case", "environment", "p_type")) %>%
  # Add deployment ID column as 'all'
  mutate(dep_id = "all")

# Bind together deployment and overall summary statistics
summary <- rbind(deployment_summary, overall_summary)
```
...
# Add dep_id, environment, p_type, to redu dataframe 
summary_redu <- summary_wide %>%
  select(dep_id, environment, p_type) %>%
  cbind(summary_redu)
```

```{r}
write_csv(summary, "data_summarized/summary_SCOPE_modulair_raw.csv")
write_csv(summary_redu, "data_summarized/summary_SCOPE_modulair_redu.csv")
```
