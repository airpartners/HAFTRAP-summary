...
time_map <- read_excel("data_raw/Olin/Olin_field_notes.xlsx", range = "A3:I25") %>%
  ...
  select(participant_id, starts_with("dt_")) %>%
...
```{r}
# Filter original dataframe to only include participants in the field notes
all_df <- all_df %>% filter(participant_id %in% unique(time_map$participant_id))
...
```{r}
clean_df <- case_df %>% mutate(pm1num = bin0 + bin1 + bin2, .keep = "unused") %>% 
...
df_ratio <- df_wide %>%
  mutate(environment = "ratio",
         ...
         pm10 = pm10_indoor/pm10_outdoor, pm1num = pm1num_indoor/pm1num_outdoor,
         .keep = "unused")
